#vectorized Greedy Motif Search Algorithm with pseudocounts
import numpy as np
import time
...
def GreedyMotifSearch_forPseudoCounts(list_of_seqs, k, t):
    '''
    ...
    for kmer_m in list_of_kmers_in_first:
        ...
        for seq in list_of_seqs[1:]:
            #edited to accommodate pseudo counts
            prob_m = (sum(tempMotifs)+1)/ float(len(tempMotifs)+4)
...
if __name__=="__main__":
    #read inputs
    start = time.time()
    data_file = open("rosalind_ba2e.txt").read().rstrip().split("\n")
    ...
    final_motifs = GreedyMotifSearch_forPseudoCounts(list_of_seqs, k, t)
